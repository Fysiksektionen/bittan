...
def get_session(request: Request, session_id: str) -> Response:
    ...
    try:
        ...
        return Response("SessionNotFound", status=404)
    ...
    if tickets.count() > 1:
        return Response({
            ...
            "email": payment.email,
            "chapter_event": tickets.first().chapter_event.pk, #Still only supports one chapter event per payment.
            "tickets": ticket_data
        })
    ...
    form_price = AnswerSelectedOptions.objects.filter(
        answer__ticket__in=tickets.all()
    ).aggregate(
    ...
    return Response({
            ...
            "chapter_event": ticket.chapter_event.pk,
            "tickets": ticket_data,
            "answers": answer_data
        })
